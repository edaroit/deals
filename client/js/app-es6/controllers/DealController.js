...
class DealController {
  ...
  constructor() {
    ...
    this._dealList = new Bind(
      new DealsList(),
      new DealsView($('#deals-view')),
      'add', 'empty' , 'order', 'reverseOrder');

    this._message = new Bind(
      new Message(), new MessageView($('#message-view')),
      'text');
  ...
  order(column) {
    if(this._actualOrder == column) {
      this._dealList.reverseOrder();
    } else {
      this._dealList.order((p, s) => p[column] - s[column]);
    }
